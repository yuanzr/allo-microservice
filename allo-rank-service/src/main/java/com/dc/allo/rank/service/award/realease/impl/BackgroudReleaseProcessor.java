...
@Component
@Slf4j
public class BackgroudReleaseProcessor extends AbstractReleaseProcessor {
    @Autowired
    ...
    public ReleaseResult doRelease(long uid, AwardOfPackage awardOfPackage, AwardOfPackageMessage awardOfPackageMessage) throws Exception {
        if(uid<=0||awardOfPackageMessage == null){
            log.info("背景发奖参数错误，uid:{},seqId:{},awards:{}",uid, awardOfPackageMessage.getSeqId(), JsonUtils.toJson(awardOfPackageMessage));
            return ReleaseResult.fail("背景发奖参数错误");
        }
        kafkaSender.send(KafkaTopic.Rank.DC_RANK_AWARD,KafkaTopic.EventType.DC_RANK_AWARD_BACKGROUND,awardOfPackageMessage);
        return ReleaseResult.success();
    }

    @Override
    public AwardEnums.ReleaseType getReleaseType() {
        return AwardEnums.ReleaseType.BACKGROUND;
    }
}
