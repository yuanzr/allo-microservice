...
@Component
@Slf4j
public class AvatarReleaseProcessor extends AbstractReleaseProcessor {
    @Autowired
    ...
    public ReleaseResult doRelease(long uid, AwardOfPackage awardOfPackage, AwardOfPackageMessage awardOfPackageMessage) throws Exception {
        if(uid<=0||awardOfPackageMessage == null){
            log.info("头饰发奖参数错误，uid:{},seqId:{},awards:{}",uid, awardOfPackageMessage.getSeqId(), JsonUtils.toJson(awardOfPackageMessage));
            return ReleaseResult.fail("头饰发奖参数错误");
        }
        kafkaSender.send(KafkaTopic.Rank.DC_RANK_AWARD,KafkaTopic.EventType.DC_RANK_AWARD_AVATAR,awardOfPackageMessage);
        return ReleaseResult.success();
    }

    @Override
    public AwardEnums.ReleaseType getReleaseType() {
        return AwardEnums.ReleaseType.AVATAR;
    }
}
